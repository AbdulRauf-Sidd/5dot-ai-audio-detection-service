"""Raw-SQL Postgres access for the ai_audio worker.
...
def fetch_job(conn, job_id: str) -> dict | None:
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute(
            """
            SELECT id, filename, file_key, url_source, duration, detect_ai_audio
...
def mark_processing(conn, job_id: str) -> None:
    with conn.cursor() as cur:
        cur.execute(
            "UPDATE detection_requests SET ai_audio_status = 'processing' WHERE id = %s",
            (job_id,),
        )
...
def save_result(conn, job_id: str, score: float) -> None:
    with conn.cursor() as cur:
        cur.execute(
            ...
            SET ai_audio_status = 'complete', overall_ai_audio_score = %s
            WHERE id = %s
            """,
            (score, job_id),
        )
...
def mark_failed(conn, job_id: str, error_message: str) -> None:
    with conn.cursor() as cur:
        cur.execute(
            """
            UPDATE detection_requests
            SET ai_audio_status = 'failed', error_message = %s
            WHERE id = %s
            """,
...
def update_chunk(conn, job_id: str, chunk_index: int, score: float, start: float, end: float) -> None:
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            SET ai_audio_score = %s, ai_audio_start = %s, ai_audio_end = %s
...
def other_services_done(conn, job_id: str) -> bool:
    """True if every other requested detection (video/lipsync/changes) on this
    ...
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute(
            """
            SELECT detect_ai_video, ai_video_status,
        ...
        return (
            done(row["detect_ai_video"], row["ai_video_status"])
